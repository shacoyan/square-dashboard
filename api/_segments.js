/**
 * src/lib/customerSegment.ts の Node 版 (JS ESM) ポート。
 * cron / バックフィルから呼び出される。
 * 元 TS とロジックは完全同一。整合性は api/_segments.test.js (fixture テスト) で担保。
 *
 * @typedef {{ new: number, repeat: number, regular: number, staff: number, unlisted: number }} SegmentBreakdown
 * @typedef {{ google: number, review: number, signboard: number, sns: number, unknown: number }} AcquisitionBreakdown
 * @typedef {{ id: string, customer_name: string|null, created_at_jst: string, amount: number, status: string, source: string, line_items: { name: string, quantity: string }[], discounts: any[] }} Transaction
 */

export function countCustomersByTransaction(tx) {
  const initial = { new: 0, repeat: 0, regular: 0, staff: 0, unlisted: 0 };
  const result = tx.line_items.reduce((acc, item) => {
...
export function allocateSalesByTransaction(tx) {
  ...
  if (counts.regular > 0) {
    ...
    const priority = ['new', 'repeat', 'staff'];
...
export function detectAcquisitionChannels(tx) {
  const result = { google: 0, review: 0, signboard: 0, sns: 0, unknown: 0 };
  let newQty = 0;
...
export function aggregateSegments(transactions) {
  const customers = { new: 0, repeat: 0, regular: 0, staff: 0, unlisted: 0 };
  const sales = { new: 0, repeat: 0, regular: 0, staff: 0, unlisted: 0 };
  const acquisition = { google: 0, review: 0, signboard: 0, sns: 0, unknown: 0 };
